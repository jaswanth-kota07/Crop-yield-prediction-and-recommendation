...
if not DYNAMIC_RECOMMENDATIONS_AVAILABLE and predict_fertilizer is None:
    print("Warning: Neither ML nor Dynamic Recommendation modules are available fully.")
...
def preprocess_input(input_dict: Dict[str, Any], models: Dict) -> Tuple[np.ndarray, list, Dict]:
    ...
    preprocessing_info = {
        'original_npk': (
            input_dict.get('N', input_dict.get('n', 0)), 
            input_dict.get('P', input_dict.get('p', 0)), 
            input_dict.get('K', input_dict.get('k', 0))
        ),
...
def predict_crop(X: np.ndarray, models: Dict) -> Dict[str, Any]:
    ...
    try:
        ...
        if hasattr(model, 'predict_proba'):
            ...
            top_classes = [predicted_class]
        
        # Helper to decode class index to crop name
        def decode_class(cls_idx):
            if 'crop_label_encoder' in models:
                return models['crop_label_encoder'].inverse_transform([cls_idx])[0]
            elif 'encoders' in models and 'label' in models['encoders']:
                return models['encoders']['label'].inverse_transform([cls_idx])[0]
            elif 'encoders' in models and 'crop' in models['encoders']:
                return models['encoders']['crop'].inverse_transform([cls_idx])[0]
            else:
                crop_names = [
                ...
                if 0 <= cls_idx < len(crop_names):
                    return crop_names[cls_idx]
                return 'unknown'
        
        crop_name = decode_class(predicted_class)
        
        if hasattr(model, 'predict_proba'):
            for cls_idx in top_classes:
                alternative_crops.append({
                    'crop': decode_class(cls_idx),
...
def predict_from_dict(input_dict: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        response = {
            ...
            'alternative_crops': crop_result.get('alternative_crops', [])
        }
        ...
        return {
            'error': f"{str(e)} | Stack: {traceback.format_exc()}",
            'recommended_crop': 'rice',
            'confidence': 0.0,
